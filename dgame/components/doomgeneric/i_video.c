...
// Gamma correction level to use 0..4
int usegamma = 4;
...
// fill output buffer with upscale to target width
static void cmap_to_rgb565_stretch( uint16_t* out, uint8_t* in, int stretchX )
{
    int rX = stretchX;

    for( int i = 0; i < SCREENWIDTH; i++ ) // for each pixel in a row
    {
        struct color c = colors[*in]; 
...
//
// I_FinishUpdate -- called when frame is ready in internal buffer 320x200
//
// Modified to scale from internal 320x200:8 buffer to 400x240:565 LCD
void I_FinishUpdate(void)
{
    uint8_t*  line_in  = (uint8_t *)I_VideoBuffer;    // internal doom colors - 1 byte per pixel
    uint16_t* line_out = (uint16_t *)DG_ScreenBuffer; // RGB:565

    int repeatX = SCREENWIDTH / (DOOMGENERIC_RESX % SCREENWIDTH); // repeat every n'th pixel on x
    int repeatY = SCREENHEIGHT / (DOOMGENERIC_RESY % SCREENHEIGHT); // repeat every n'th line on y
    int rY = repeatY;

    int y = SCREENHEIGHT;
    while( y-- )
    {
        cmap_to_rgb565_stretch( line_out, line_in, repeatX );

        if( --rY == 0 )
        {
            uint16_t* nextLine = line_out + DOOMGENERIC_RESX;
            memcpy( nextLine, line_out, DOOMGENERIC_RESX * sizeof(uint16_t) );
            rY = repeatY;
            line_out = nextLine;
        }

        line_in  += SCREENWIDTH;
        line_out += DOOMGENERIC_RESX;
    }

	DG_DrawFrame();
}

// this is the body of original code of I_FinishUpdate().
// I've commented it out, since I need to update and scale
// from internal 320x200:8 buffer to 400x240:565 in single pass
#if 0
    int y;
    ...
    line_out = (unsigned char *)DG_ScreenBuffer; // tagrget LCD 400x240
    ...
    DG_DrawFrame(); // send frame to lcd
}
#endif // #if 0
...
void I_SetPalette( byte* palette )
{
    ...
    /* performance boost map to the right pixel format over here! */
    for (i=0; i<256; ++i ) {
        colors[i].a = 0;
