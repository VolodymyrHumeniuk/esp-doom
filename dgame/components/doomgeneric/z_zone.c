  ...
  void*     user )
{
    ...
    if (user == NULL && tag >= PU_PURGELEVEL)
        I_Error ("Z_Malloc"); //  purgable blocks
...
void Z_CheckHeap (void)
{
    ...
    if ( (byte *)block + block->size != (byte *)block->next)
        I_Error ("Z_CheckHeap: 7\n"); //block size does not touch the next block\n");

    if ( block->next->prev != block)
        I_Error ("Z_CheckHeap: 8\n"); // next block doesn't have proper back link\n");

    if (block->tag == PU_FREE && block->next->tag == PU_FREE)
        I_Error ("Z_CheckHeap: 9\n"); // two consecutive free blocks\n");
    }
}
...
void Z_ChangeTag2(void *ptr, int tag, char *file, int line)
{
    ...
    if (block->id != ZONEID)
        I_Error("%s:%i: Z_ChangeTag: ZONEID!", file, line);

    if (tag >= PU_PURGELEVEL && block->user == NULL)
        I_Error("%s:%i: Z_ChangeTag", file, line); // : an owner is required for purgable blocks
